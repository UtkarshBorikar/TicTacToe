...
public class TTTPanel extends JPanel implements MouseListener {
    ...
    public void draw(Graphics g) {
        ...
        for (int i = 1; i <= 3; i++) {
            g2.drawLine(UNIT_SIZE * i, 0, UNIT_SIZE * i, SCREEN_HEIGHT);
            g2.drawLine(0, UNIT_SIZE * i, SCREEN_WIDTH, UNIT_SIZE * i);
        }
        for (int i = 0; i < TTT.length; i++) {
            if (TTT[i] != 2) {
                drawShape(g, ((i % 3 + 1) * 2 - 1) * 100, (((i / 3) + 1) * 2 - 1) * 100, TTT[i]);
            }
        }
        winScreen(g);
    }
    ...
    public void cross(Graphics g, int x, int y) {
        ...
        g2.setStroke(new BasicStroke(3));
        g2.drawLine(25 + x - (UNIT_SIZE / 2), 25 + y - (UNIT_SIZE / 2), -25 + x + (UNIT_SIZE / 2),
                -25 + y + (UNIT_SIZE / 2));
        g2.setStroke(new BasicStroke(3));
        g2.drawLine(-25 + x + (UNIT_SIZE / 2), 25 + y - (UNIT_SIZE / 2), 25 + x - (UNIT_SIZE / 2),
                -25 + y + (UNIT_SIZE / 2));
    ...
    public void circle(Graphics g, int x, int y) {
        ...
        g2.setStroke(new BasicStroke(3));
        g2.drawOval(25 + x - (UNIT_SIZE) / 2, 25 + y - (UNIT_SIZE) / 2, UNIT_SIZE - 50, UNIT_SIZE - 50);
    }
    ...
    public void checkDub() {
        for (int i = 0; i < 3; i++) {
            if (TTT[3 * i] == TTT[3 * i + 1] && TTT[3 * i + 1] == TTT[3 * i + 2] && TTT[3 * i] != 2) {
                win = TTT[3 * i];
            } else if (TTT[i] == TTT[i + 3] && TTT[i + 3] == TTT[i + 6] && TTT[i] != 2) {
                win = TTT[i];
            }
        }
        if ((TTT[0] == TTT[4] && TTT[4] == TTT[8]) || (TTT[2] == TTT[4] && TTT[4] == TTT[6])) {
            win = TTT[4];
        }
    }
    ...
    public void mouseClicked(MouseEvent e) {
        if (check) {
            ...
            String coords = "112131122232132333";

            Boolean doTurn = true;
            big:for (int i = 0; i < 9; i++) {
